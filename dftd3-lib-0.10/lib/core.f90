...
contains
  ...
  subroutine edisp(max_elem,maxc,n,xyz,iz,c6ab,mxc,r2r4,r0ab,rcov, &
    ...
    !$omp reduction(+:e6) &
    !$omp shared(iz,xyz,c6ab,r0ab,alp6,rs6,n) &
    !$omp private(iat, jat, dx,dy,dz, r2, r, c6,damp6, r6)
      do iat=1,n-1
        do jat=iat+1,n
    ...
    !$omp end parallel do
      ...
      !$omp parallel do default(none) &
      !$omp reduction(+:e6, e8) &
      !$omp shared(dmp, a1,a2,icomp,noabc,abcthr, maxc,max_elem,iz,xyz,r2r4,r0ab, c6ab, cn, mxc, cc6ab, &
      !$omp& r2ab, version, rs6, rs8, n, rthr, alp6, alp8) &
      !$omp private(iat, jat, ij, rr, tmp, tmp2, &
      !$omp& dx,dy,dz, r2, r, c6, c8, damp6, damp8, r6, r8)
      do iat=1,n-1
        do jat=iat+1,n
          dx=xyz(1,iat)-xyz(1,jat)
          dy=xyz(2,iat)-xyz(2,jat)
      ...
      !$omp parallel do default(none) &
      !$omp reduction(+:e63) &
      !$omp shared(r2ab, cc6ab, dmp, icomp, lin, &
      !$omp& version, rs6, rs8, n, rthr, alp6, alp8) &
      !$omp private(iat, jat, ij, kat, ik, jk, rav, tmp, &
      !$omp& t1, t2, t3, tmp2, ang,c9)
      do iat=1,n
        do jat=1,iat-1
          ij=lin(jat,iat)
  ...
  subroutine gdisp(max_elem,maxc,n,xyz,iz,c6ab,mxc,r2r4,r0ab,rcov, &
    ...
    real(wp), intent(in) :: xyz(:,:)
    real(wp) c6ab(max_elem,max_elem,maxc,maxc,3)
    ! Allouche real(wp) g(3,*),s6,s18,rcov(max_elem)
    real(wp), intent(inout) :: g(:, :)
    real(wp) s6,s18,rcov(max_elem)
    ...
    if (num) then
      ...
      !$omp parallel do default(none) &
      !$omp shared(g,n,xyz,step,iz, cn_thr, rthr, noabc, version, alp8, rs8, &
      !$omp&  alp6, rcov, mxc, maxc, max_elem, s6, s18, rs6, rabc, r0ab,c6ab) &
      !$omp private(xyzTmp,dispr, displ, labc, e6abc, e12, e10, e8, e6)
      do i=1,n
        do j=1,3
    ...
    if (version.eq.2)then
      ...
      disp=0
      !$omp parallel do default(none) &
      !$omp reduction(+:g,disp) &
      !$omp shared(xyz, n, iz, rthr, &
      !$omp&  alp6,  s6, rs6, r0ab,c6ab) &
      !$omp private(iat, jat,R0,c6, dx, dy, dz, r2, r235,r, damp1, damp6, &
      !$omp& tmp1, tmp2, gx1, gy1, gz1, gx2, gy2, gz2)
      do iat=1,n-1
        do jat=iat+1,n
    ...
    elseif ((version.eq.3).or.(version.eq.5)) then
      ...
      kk=0
      !$omp parallel do default(none) &
      !$omp reduction(+:disp,dc6i) &
      !$omp shared(drij, n,xyz, c6abc, dc6ij, iz, rthr, version, s6, s8, rs6, rs8, alp6, alp8, &
      !$omp&  maxc,max_elem,c6ab,mxc, cn, abcthr, noabc, r3abc, r0ab, r2abc, abccalc) &
      !$omp private(iat, jat, rij, r, r2, r0, r42, r6, r7, r8, r9, damp6, damp8, &
      !$omp& tmp1, tmp2, t6, t8, dc6_rest, dc6iji, dc6ijj,  c6, linij) 
      do iat=1,n
        do jat=1,iat-1
          rij=xyz(:,jat)-xyz(:,iat)
          r2=sum(rij*rij)
    ...
    elseif ((version.eq.4).or.(version.eq.6)) then
      ...
      !$omp parallel do default(none) &
      !$omp reduction(+:disp, dc6i) &
      !$omp shared(drij, a1,a2, n,xyz, c6abc, dc6ij, iz, rthr, version, s6, s8, rs6, rs8, alp6, alp8, &
      !$omp&  maxc,max_elem,c6ab,mxc, cn, abcthr, noabc, r3abc, r0ab, r2abc, abccalc) &
      !$omp private(iat, jat, kat, r2ij, dc6iji, dc6ijj, linij, c9, rav, &
      !$omp& rijk3, rav3, r, tmp1, tmp2, dang, ang, angr9, dc6_rest, t6, t8, r2,r4,r6,r8,R0,c6,rij,r42) 
      do iat=1,n
        do jat=1,iat-1
          rij=xyz(:,jat)-xyz(:,iat)
          r2=sum(rij*rij)
    ...
    if (.not.noabc)then
      ...
      !HERE
      !$omp parallel do default(none) &
      !$omp reduction(+:dc6i, disp, eabc) &
      !$omp shared(drij, n,xyz, c6abc, dc6ij, iz, rthr, version, s6, s8, rs6, rs8, alp6, alp8, &
      !$omp&  maxc,max_elem,c6ab,mxc, cn, abcthr, noabc, r3abc, r0ab, r2abc, abccalc) &
      !$omp private(iat, jat, kat, r2ij, linik, linjk, r2jk, r2ik, c9, rav, fdmp, mijk, imjk, ijmk, &
      !$omp& rijk3, rav3, r, dfdmp, tmp1, tmp2, dang, ang, angr9, dc9, dc6_rest, linij) 
      do iat=1,n
        do jat=1,iat-1
          linij=lin(iat,jat)
    ...
    !$omp parallel do default(none) &
    !$omp reduction(+:g) &
    !$omp shared(rcov, drij, dc6i, iz, xyz, cn_thr) &
    !$omp private(iat, expterm,r , r2, dcn, x1, linij, rcovij, gtmp, rij)
    do iat=2,n
      gtmp=0.0
      do jat=1,iat-1
        linij=lin(iat,jat)
...
999 continue
  ...
  subroutine ncoord(natoms,rcov,iz,xyz,cn,cn_thr)
   ...
   !$omp parallel do default(none) &
   !$omp shared(natoms,iz,xyz,rcov,cn, cn_thr) &
   !$omp private(i, xn,iat, dx,dy,dz, r2, r, rr, rco,damp)
    do i=1,natoms
      xn=0.0d0
      do iat=1,natoms
        if (iat.ne.i)then
  ...
  subroutine pbcncoord(natoms,rcov,iz,xyz,cn,lat,rep_cn,crit_cn)
   ...
   !$omp parallel do default(none) &
   !$omp shared(natoms,rep_cn,iz,xyz,lat, crit_cn, rcov,cn) &
   !$omp private(i, xn,iat, dx,dy,dz, rr, r, rco,damp, taux, tauy, tauz, tau)
    do i=1,natoms
      xn=0.0d0
      do iat=1,natoms
  ...
  subroutine pbcedisp(max_elem,maxc,n,xyz,iz,c6ab,mxc,r2r4,r0ab,&
    ...
    if (version.eq.2)then
      ...
      !$omp parallel do default(none) &
      !$omp reduction(+:e6) &
      !$omp shared(iz,xyz,c6ab,r0ab,alp6,rs6,n, rep_vdw,lat, rthr) &
      !$omp private(iat, jat, dx,dy,dz, r2, r, c6,damp6, r6, taux, tauy, tauz, tau)
      do iat=1,n-1
        do jat=iat+1,n
          c6=c6ab(iz(jat),iz(iat),1,1,1)
      ...
      !$omp parallel do default(none) &
      !$omp reduction(+:e6) &
      !$omp shared(rep_vdw,iz,xyz,c6ab,r0ab,alp6,rs6,n,lat, rthr) &
      !$omp private(iat, jat, dx,dy,dz, r2, r, c6,damp6, r6, taux, tauy, tauz, tau)
      do iat=1,n
        jat=iat
        c6=c6ab(iz(jat),iz(iat),1,1,1)
      ...
      !$omp parallel do default(none) &
      !$omp reduction(+:e6, e8) &
      !$omp shared(noabc,maxc,max_elem,iz,xyz,r2r4,r0ab, c6ab, cn, mxc, cc6ab, &
      !$omp& version, rs6, rs8, n, rep_vdw,lat, rthr, alp6, alp8) &
      !$omp private(iat, jat, ij, R0, rr, tmp, &
      !$omp& dx,dy,dz, r2, r, c6, c8, damp6, damp8, r6, r8, taux, tauy, tauz, tau)
      do iat=1,n-1
        do jat=iat+1,n
          ! get C6
          call getc6(maxc,max_elem,c6ab,mxc,iz(iat),iz(jat),&
      ...
      !$omp parallel do default(none) &
      !$omp reduction(+:e6, e8,counter) &
      !$omp shared(a1,a2,noabc,maxc,max_elem,iz,xyz,r2r4,r0ab, c6ab, cn, mxc, cc6ab, &
      !$omp& version, rs6, rs8, n, rep_vdw,lat, rthr, alp6, alp8) &
      !$omp private(iat, jat, ij, r42, rxyz, R0, rr, tmp, dxyz, bj_dmp6, bj_dmp8, &
      !$omp& dx,dy,dz, r2, r, c6, c8, damp6, damp8, r6, r8, taux, tauy, tauz, tau)
      do iat=1,n
        do jat=iat+1,n
          ! get C6
  ...
  subroutine pbcgdisp(max_elem,maxc,n,xyz,iz,c6ab,mxc,r2r4,r0ab,&
    ...
    if (num) then
      ...
      xyzTmp = xyz
      !$omp parallel do default(none) &
      !$omp shared(g,n,xyz,step,iz, rep_cn, crit_cn, rep_v, rthr, lat, noabc, version, alp8, rs8, &
      !$omp&  alp6, rcov, mxc, maxc, max_elem, s6, s18, rs6, rabc, gabc, tau, r0ab,c6ab) &
      !$omp private(xyzTmp,dispr, displ, labc, e6abc, e12, e10, e8, e6)
      do i=1,n
        do j=1,3
          xyzTmp(j,i)=xyz(j,i)+step
    ...
    if (version.eq.2)then
      ...
      !$omp parallel do default(none) &
      !$omp reduction(+:g,sigma, disp) &
      !$omp shared(xyz, n,iz, rep_v, lat, rthr, &
      !$omp&  alp6,  s6, rs6, r0ab,c6ab) &
      !$omp private(iat, jat,R0,c6, taux, tauy, tauz, tau, dxyz, r2, r235,r, damp1, damp6, term, tmp1, tmp2)
      do iat=1,n-1
        do jat=iat+1,n
          R0=r0ab(iz(jat),iz(iat))*rs6
          c6=c6ab(iz(jat),iz(iat),1,1,1)*s6
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      !$omp parallel do default(none) &
      !$omp reduction(+:disp, dc6i) &
      !$omp shared(drij, dc6_rest_sum,  c6save, dc6ij, r2r4,  n,xyz,iz, crit_cn, rep_v, rthr, lat, version, alp8, rs8, &
      !$omp&  alp6, rcov, mxc, maxc, max_elem, s6, s8, s18, rs6, rabc, r0ab,c6ab, cn) &
      !$omp private(iat, jat, dc6ijj, dc6iji, rij, linij, &
      !$omp& damp6, damp8, tmp1, tmp2, r6, r7, r8, r9, r, r2, t6, t8, r0, r42, rcovij, c6,  &
      !$omp& taux, tauy, tauz, tau, dc6_rest)
      do iat=1,n
        call get_dC6_dCNij(maxc,max_elem,c6ab,mxc(iz(iat)),&
...
!!!!!!!!!!!!!!!!!!!!!!!
      ...
      !$omp&  a1, a2) &
      !$omp private(iat, jat, dc6ijj, dc6iji, rij, linij, &
      !$omp& damp6, damp8, tmp1, tmp2, r6, r7, r8, r9, r, r2, r4, t6, t8, R0, r42, rcovij, c6,  &
      !$omp& taux, tauy, tauz, tau, dc6_rest)
      do iat=1,n
        call get_dC6_dCNij(maxc,max_elem,c6ab,mxc(iz(iat)),&
            & mxc(iz(iat)),cn(iat),cn(iat),iz(iat),iz(iat),&
...
!!!!!!!!!!!!!!!!!!!!!!!
    if (.not.noabc) then
      ...
      !$omp parallel do default(none) &
      !$omp reduction(+:disp, eabc, dc6i) &
      !$omp shared(drij, c6save, dc6ij, r2r4,  n,xyz,iz, crit_cn, rep_v, rthr, lat, version, alp8, rs8, &
      !$omp&  alp6, rcov, mxc, maxc, max_elem, s6, s8, s18, rs6, rabc, r0ab,c6ab, cn, &
      !$omp&  a1, a2, sr9, abcthr, rep_cn) &
      !$omp private(dc6ijj, dc6iji, rij, ijvec, linij, linjk, linik, ikvec, jkvec, c6ik, c6jk, c9,  &
      !$omp& damp6, damp8, tmp1, tmp2, r6, r7, r8, r9, r, r2, r4, t6, t8, R0, r42, rcovij, c6,  &
      !$omp& taux, tauy, tauz, tau, dc6_rest, c6ij, rij2, rr0ij, rjk2, rik2, rr0ik, rr0jk, r0av, jtau, &
      !$omp& ktau, ang, dang, dfdmp, dc9, damp9, repmin, repmax, dumvec, geomean2, geomean, geomean3)
      do iat=3,n
        do jat=2,iat-1
          linij=lin(iat,jat)
          ijvec=xyz(:,jat)-xyz(:,iat)
      ...
      !$omp parallel do default(none) &
      !$omp reduction(+:disp, eabc, drij, dc6i) &
      !$omp shared(c6save, dc6ij, r2r4,  n,xyz,iz, crit_cn, repv, rthr, lat, version, alp8, rs8, &
      !$omp&  alp6, rcov, mxc, maxc, max_elem, s6, s8, s18, rs6, rabc, r0ab,c6ab, cn, &
      !$omp&  a1, a2, sr9, abcthr, rep_cn) &
      !$omp private(iat, jat, dc6ijj, dc6iji, rij, ijvec, linij, linjk, linik, ikvec, jkvec, c6ik, c6jk, c9,  &
      !$omp& damp6, damp8, tmp1, tmp2, r6, r7, r8, r9, r, r2, r4, t6, t8, R0, r42, rcovij, c6,  &
      !$omp& taux, tauy, tauz, tau, dc6_rest, c6ij, rij2, rr0ij, rjk2, rik2, rr0ik, rr0jk, r0av, jtau, &
      !$omp& ktau, ang, dang, dfdmp, dc9, damp9, repmin, repmax, dumvec, geomean2, geomean, geomean3)
      do iat=2,n
      ...
      !$omp&  alp6, rcov, mxc, maxc, max_elem, s6, s8, s18, rs6, rabc, r0ab,c6ab, cn, &
      !$omp&  a1, a2, sr9, abcthr, rep_cn) &
      !$omp private(iat, jat, kat, dc6ijj, dc6iji, rij, ijvec, linij, linjk, linik, ikvec, jkvec, c6ik, c6jk, c9,  &
      !$omp& damp6, damp8, tmp1, tmp2, r6, r7, r8, r9, r, r2, r4, t6, t8, R0, r42, rcovij, c6,  &
      !$omp& taux, tauy, tauz, tau, dc6_rest, c6ij, rij2, rr0ij, rjk2, rik2, rr0ik, rr0jk, r0av, jtau, &
      !$omp& ktau, ang, dang, dfdmp, dc9, damp9, repmin, repmax, dumvec, geomean2, geomean, geomean3)
      do iat=2,n
    ...
    !$omp shared(rcov, rthr, drij, dc6i, iz, xyz, rep_v,lat, crit_cn) &
    !$omp private(iat, jat,  &
    !$omp& expterm,r , rij, r2, dcnn, x1, vec,   &
    !$omp& taux, tauy, tauz, tau, linij, rcovij)
    do iat=2,n
      do jat=1,iat-1
        linij=lin(iat,jat)
        rcovij=rcov(iz(iat))+rcov(iz(jat))
    ...
    !$omp parallel do default(none) &
    !$omp reduction(+:sigma) &
    !$omp shared(rcov, drij, dc6i, iz, xyz, rep_v,lat, crit_cn) &
    !$omp private(iat, &
    !$omp& expterm,r , r2, dcnn, x1, vec, vec2,   &
    !$omp& taux, tauy, tauz, tau, linij, rcovij)
    do iat=1,n
      rcovij=rcov(iz(iat))+rcov(iz(iat))
      do taux=-rep_v(1),rep_v(1)
