...
void Mode::setupGlobal()
{
	if (settings.keywordExists("verbose"))
	{
		verbose = atoi(settings["verbose"].c_str());
	}
	if (settings.keywordExists("conv_phys_to_bohr"))
	{
		convToBohr = atof(settings["conv_phys_to_bohr"].c_str());
	}
	if (settings.keywordExists("conv_phys_to_hartree"))
	{
		convToHartree = atof(settings["conv_phys_to_hartree"].c_str());
	}
}
void Mode::setupExternal()
{
	log << "\n";
	log << "*** SETUP: External potential *******"
		"**************************************\n";
	log << "\n";
	std::string script = "unknown";
	std::string prefixFileName = "external";

	if (settings.keywordExists("external_script"))
	{
		script = settings["external_script"];
		log << "script      : ";
		log << script;
		log << "\n";
	}
	
	if (settings.keywordExists("external_prefix"))
	{
		prefixFileName = settings["external_prefix"];
		log << "prefixFileName      : ";
		log << prefixFileName;
		log << "\n";
	}
	external = External();
	external.setscript(script);
	external.setprefixfilename(prefixFileName);
	if(normalize) external.setnormalization( meanEnergy,  convEnergy, convLength, convCharge);
	external.setcontoatomicunit(convToHartree, convToBohr);
...
void Mode::setupRepulsion()
{
	log << "\n";
	log << "*** SETUP: Repulsive potential *******"
		"**************************************\n";
	log << "\n";
	std::string type = "no";
	double rIn = 0;
	double rOut = maxCutoffRadius / convLength;// rOut in Phys unit

	if (settings.keywordExists("repulsion_type"))
	{
		type = settings["repulsion_type"].c_str();
		log << "repulsion type     : ";
		log << type;
		log << "\n";
	}
	
	if (settings.keywordExists("repulsion_rin"))
	{
		rIn = atof(settings["repulsion_rin"].c_str());
		if(rIn<0) rIn = 0;
		log << strpr("rIn     : %f\n", rIn);
	}
	if (settings.keywordExists("repulsion_rout"))
	{
		rOut = atof(settings["repulsion_rout"].c_str());
		if(rOut<rIn) 
		{
		}
		log << strpr("rOut     : %f\n", rOut);
	...
	rep = Repulsion(type);
	if(normalize) rep.setnormalization( meanEnergy,  convEnergy, convLength, convCharge);
	rep.setcontoatomicunit(convToHartree, convToBohr); // before setrIn
	rep.setrIn(rIn);
	rep.setrOut(rOut);
...
void Mode::setupDFTD3()
{
	...
	if (settings.keywordExists("dftd3_verbose"))
	{
		verbose = atoi(settings["dftd3_verbose"].c_str());
		log << strpr("Verbose     : %d\n", verbose);
	}
	
	if (settings.keywordExists("dftd3_func"))
	{
		func = settings["dftd3_func"];
		log << "DFTD3 func :"<<func<<"\n";
	}
	if (settings.keywordExists("dftd3_version"))
	{
		version = atoi(settings["dftd3_version"].c_str());
		log << strpr("Version     : %d\n", version);
	}
	transform(func.begin(), func.end(), func.begin(), ::tolower);
	dftD3 = DFTD3(func,version,verbose);
	if(func==string("custom"))
	{
		if (settings.keywordExists("dftd3_s6"))
		{
			double s6 = atof(settings["dftd3_s6"].c_str());
			dftD3.sets6(s6);
		}
		if (settings.keywordExists("dftd3_rs6"))
		{
			double rs6 = atof(settings["dftd3_rs6"].c_str());
			dftD3.setrs6(rs6);
		}
		if (settings.keywordExists("dftd3_s18"))
		{
			double s18 = atof(settings["dftd3_s18"].c_str());
			dftD3.sets18(s18);
		}
		if (settings.keywordExists("dftd3_rs18"))
		{
			double rs18 = atof(settings["dftd3_rs18"].c_str());
			dftD3.setrs18(rs18);
		}
		if (settings.keywordExists("dftd3_alp6"))
		{
			double alp6 = atof(settings["dftd3_alp6"].c_str());
			dftD3.setalp6(alp6);
		}
		dftD3.printParameterNames(log);
	}

	log << "*****************************************"
		"**************************************\n";
        dftD3.checkVersion(log);
	if(normalize) external.setnormalization( meanEnergy,  convEnergy, convLength, convCharge);
	external.setcontoatomicunit(convToHartree, convToBohr);
...
#endif
    setupNeuralNetwork();
    setupRepulsion();// After nomalization
    setupExternal(); // After nomalization
...
void Mode::calculateEnergy(Structure& structure) const
{
    ...
    dftD3.calculateEnergy(structure);
    rep.calculateEnergy(structure);
    external.calculateEnergy(structure);

    structure.energy = structure.energyShort + structure.energyElec + structure.energyDFTD3 + structure.energyRep + structure.energyExternal ;

    if (dftD3.getVerbose()>0 or verbose>0)
    {
    	cout << strpr("Electrostatic energy: %24.16E\n", structure.energyElec);
    	cout << strpr("Short-range   energy: %24.16E\n", structure.energyShort);
    	cout << strpr("DFTD3         energy: %24.16E\n", structure.energyDFTD3);
    	cout << strpr("Repulsion     energy: %24.16E\n", structure.energyRep);
    	cout << strpr("External      energy: %24.16E\n", structure.energyExternal);
...
void Mode::calculateForces(Structure& structure) const
{
    ...
    dftD3.calculateForces(structure);
    rep.calculateForces(structure);
    external.calculateForces(structure);
	...
	/*
    	string XYZ[] = {"X","Y","Z"};
    	for(int k =0;k<3;k++) log << strpr("%20s ",XYZ[k].c_str());
    	for(int k =0;k<3;k++) log << strpr("FElec %14s",XYZ[k].c_str());
    	for(int k =0;k<3;k++) log << strpr("F %18s",XYZ[k].c_str());
    	log<<"\n";
    	for (size_t i = 0; i < structure.atoms.size(); ++i)
    	{
            auto &ai = structure.atoms[i];
	    for(int k =0;k<3;k++) log << strpr("%20.10f ",ai.r[k]);
	    for(int k =0;k<3;k++) log << strpr("%20.10f ",ai.fElec[k]);
	    for(int k =0;k<3;k++) log << strpr("%20.10f ",ai.f[k]);
    	    log<<"\n";
   	}
	*/
    	// Print forces for control
	...
	cout.setf(ios::scientific);
    	for(int k =0;k<3;k++) cout << setw(w)<<XYZ[k];
    	for(int k =0;k<3;k++) cout << setw(w)<<"FElec"+XYZ[k];
    	for(int k =0;k<3;k++) cout << setw(w)<<"F"+XYZ[k];
    	cout<<endl;
    	for (size_t i = 0; i < structure.atoms.size(); ++i)
    	{
            auto &ai = structure.atoms[i];
	    for(int k =0;k<3;k++) cout << setw(w)<<setprecision(8)<<ai.r[k]<<" ";
	    for(int k =0;k<3;k++) cout << setw(w)<<setprecision(8)<<ai.fElec[k]<<" ";
	    for(int k =0;k<3;k++) cout << setw(w)<<setprecision(8)<<ai.f[k]<<" ";
	    cout<<endl;
   	}
	cout << "======================================================"
	        "======================================================"
	        "===================================\n";
	dftD3.printForces(structure,log);
	rep.printForces(structure,log);
	external.printForces(structure,log);
    }
    return;
}
...
void Mode::evaluateNNP(Structure& structure, bool useForces, bool useDEdG)
{
	...
	*/
    if (nnpType == NNPType::HDNNP_4G || nnpType == NNPType::HDNNP_Q)
     		calculateCharge(structure);
    if (useForces) 
		calculateForces(structure); // some methods calculate energy with forces
    calculateEnergy(structure);
}
